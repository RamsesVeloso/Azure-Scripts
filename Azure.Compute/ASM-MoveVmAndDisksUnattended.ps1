...
ForEach($disk in $allDisks)
{
    ...
    $targetBlob = Start-AzureStorageBlobCopy -SrcContainer vhds -SrcBlob $blobName -Context $sourceContext `
        -DestContainer vhds -DestBlob $blobName -DestContext $destContext -Force
...
Add-AzureDisk -OS $sourceOSDisk.OS -DiskName $sourceOSDisk.DiskName -MediaLocation $destOSDisk.ICloudBlob.Uri.AbsoluteUri
...
## Delete source VM before recreating it in new location
Select-AzureSubscription -SubscriptionId $sourceSubscriptionId
Remove-AzureVM -ServiceName $sourceServiceName -Name $sourceVmName -DeleteVHD -Verbose
